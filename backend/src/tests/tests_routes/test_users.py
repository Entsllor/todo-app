...
from src.tests.conftest import DEFAULT_USER_PASSWORD, USER_CREATE_DATA, urls


def test_create_user(client, session):
    response = client.post(urls.create_user, json=USER_CREATE_DATA)
...
def test_failed_login_wrong_password(default_user, client):
    response = client.post(urls.login, json={'login': default_user.login, 'password': "__WRONG_PASSWORD"})
...
def test_failed_login_user_does_not_exist(default_user, client):
    response = client.post(urls.login, json={'login': "__WRONG_USERNAME", 'password': DEFAULT_USER_PASSWORD})
    assert response.status_code == 401, response.text
    with pytest.raises(ValidationError):
        assert AuthTokensOut(**response.json)  # validate response content
...
def test_revoke_tokens(client, token_pair, default_user):
    ...
    response = client.post(urls.revoke)
    assert response.status_code == 200, response.text
